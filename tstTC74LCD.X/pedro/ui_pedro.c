...
static uint8_t thrLum = ALAL;    // 0 - 3
...
static uint8_t prevTempCond = 0;
static uint8_t prevLumCond  = 0;
static volatile uint8_t pwm_seconds_left = 0;
...
{
    AlarmPwm_Tick1s();      // <--- required for stopping after TALA seconds
    time_inactive++; 
...
void RenderRecords(void){
    ...
    sprintf(line1,"%02u:%02u:%02u %02uC %u", records[record_index].clk_hh, records[record_index].clk_mm, records[record_index].clk_ss, records[record_index].temp, records[record_index].lum);
    LCDcmd(0x80); LCDpos(0,0);LCDstr(line1); while (LCDbusy());
    sprintf(line2,"%02u:%02u:%02u %02uC %u", records[record_index+1].clk_hh, records[record_index+1].clk_mm, records[record_index+1].clk_ss, records[record_index+1].temp, records[record_index+1].lum);
...
void RenderNormal(void)
{
    ...
    sprintf(line2,"%02d C         L %1u ",temp,light);
    //LCDcmd(0x80); LCDpos(0,0);LCDstr(line1); while (LCDbusy());
    LCDcmd(0xC0); LCDpos(8,0);LCDstr(line2); while (LCDbusy());
}
...
void EvaluateTempLumAlarms(void)
{
    ...
    // Threshold indicator LEDs (always show current state)
    // Replace with your correct pin macros/bits if different.
...
if (lumCond)  IO_RA4_SetHigh(); else IO_RA4_SetLow();
    ...
    // Rising-edge trigger: only when condition becomes true
    uint8_t newTrigger = 0;

    if (tempCond && !prevTempCond) { alarmFlagT = 1; newTrigger = 1; }
    if (lumCond  && !prevLumCond)  { alarmFlagL = 1; newTrigger = 1; }

    if (newTrigger && pwm_seconds_left == 0) {
        pwm_seconds_left = TALA;
        PWM6_LoadDutyValue(512);   // example duty; keep what your project uses
        PWM6_Start();
    }
...
static void StartAlarmPwmForTALA(void)
{
    pwm_seconds_left = TALA;
    PWM6_LoadDutyValue(512);     // example 50% (adjust to your PWM module)
    PWM6_Start();
}

void AlarmPwm_Tick1s(void)
{
    if (pwm_seconds_left == 0) return;

    if (--pwm_seconds_left == 0) {
        PWM6_Stop();
    }
}
