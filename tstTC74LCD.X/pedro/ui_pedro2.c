...
static uint8_t alarmFlagC, alarmFlagT, alarmFlagL; // letters C/T/L until S1 clears
...
static record_field_t records[4]; // [0 max temp, 1 min temp, 2 max lum, 3 min lum]
...
static uint8_t thrTemp   = ALAT; // 0..50
...
/* -------------------- Config field stepping (FIXES your config bug) --------------------
   This removes the old "jump" that prevented reaching A/a and R.
*/
...
void OnS1Pressed(void)
{
    ...
    if (mode == UI_CONFIG) {
        // Advance to next config field
        cfg_field_t next = cfg_next(field);

        // If we were on CF_RESET and advance, exit config (matches your old CF_DONE behavior)
        if (field == CF_RESET) {
            mode  = UI_NORMAL;
            field = CF_CLK_HH;
            return;
...
void OnS2Pressed(void)
{
    ...
    if (mode == UI_CONFIG) {
        switch (field) {
            ...
            case CF_CTL_C:       field = CF_CTL_C_HH; break; // enter sub-edit
            ...
            case CF_CTL_T:       thrTemp = (thrTemp + 1) % 51; break; // 0..50
            case CF_CTL_L:       thrLum  = (thrLum  + 1) % 4;  break; // 0..3

            case CF_ALARM_EN:    alarmsEnabled ^= 1; break;

            case CF_RESET:
                // Reset records (keeps your progress, removes "not implemented")
...
void RenderRecords(void)
{
    ...
    // Keep your original LCD write pattern (so we don't break lcd.h behavior)
    // Show two records (record_index and record_index+1)
    record_field_t *r1 = &records[record_index];
    record_field_t *r2 = &records[record_index + 1];

    // line: "hh:mm:ss ttC L x"
...
{
    // this is your 1 Hz UI tick
    Alarm_BeepTick1s();
    time_inactive++;

    // Time alarm check: runs once per second.
    // IMPORTANT: this uses current hh/mm/ss; it will trigger on exact match.
    {
...
static uint8_t readLuminosityLevel(void)
{
    ...
    uint8_t level = (uint8_t)(raw >> 8);   // top 2 bits -> 0..3
