...
static uint8_t alarmFlagC, alarmFlagT, alarmFlagL; // alarm “event” flags (C/T/L) cleared by S1
...
static inline void Alarm_SuppressBegin_1s(void)
{
    ...
    // turn OFF threshold LEDs immediately
    IO_RA5_SetLow();
    IO_RA4_SetLow();

    // reset edges so conditions can retrigger after mute window
    prevTempCond  = 0;
    prevLumCond   = 0;
    prevClockCond = 0;
}
...
void UI_OnTick1s(void)
{
    ...
    if (alarm_suppress_1s) {
        ...
        // reset edges so it can retrigger after mute window
        prevTempCond  = 0;
        prevLumCond   = 0;
        prevClockCond = 0;

        alarm_suppress_1s = 0; // consume mute
    } else {
        Alarm_BeepTick1s();
...
void ClearRecords(void)
{
    ...
    records[3].temp = 0;    records[3].lum = 3;   // Lmin
